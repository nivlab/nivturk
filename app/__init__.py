...
if cfg['FLASK']['SECRET_KEY'] == "PLEASE_CHANGE_THIS":
    ...
    def index():
        ...
        info = dict(
        workerId     = gen_code(5),#altering for geha to avoid assignment_IDs #request.args.get('workerId'),        # MTurk metadata
        assignmentId = gen_code(5),#request.args.get('assignmentId'),    # MTurk metadata
        hitId        = gen_code(5),#request.args.get('hitId'),           # MTurk metadata
        ...
        if info['platform'] in ['android','iphone','ipad','wii']:
            ...
            ## commented this out for use at geha
            '''
