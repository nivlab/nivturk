...
def stage01():
    ...
    ## Case 1: previously completed both experiments.
    elif session.get('complete', False) == 2:

        ## Update metadata.
        session['WARNING'] = "Revisited stage01."
        write_metadata(session, ['WARNING'], 'a')

        ## Redirect participant to complete page.
        return redirect(url_for('complete.complete'))

    ## Case 2: previously completed stage01.
    elif session.get('complete', False) == 1:
    ...
    ## Case 3: repeat visit.
        ...
        ## Present experiment.
        return render_template('stage01.html', workerId=session['workerId'], assignmentId=session['assignmentId'], hitId=session['hitId'], code_success=session['code_success'], code_reject=session['code_reject'])

    ## Case 4: first visit.
    else:

        ## Update participant metadata.
        session['stage01'] = True
        write_metadata(session, ['stage01'], 'a')

        ## Present experiment.
        return render_template('stage01.html', workerId=session['workerId'], assignmentId=session['assignmentId'], hitId=session['hitId'], code_success=session['code_success'], code_reject=session['code_reject'])

@bp.route('/stage01', methods=['POST'])
def pass_message():
...
@bp.route('/redirect_stage02', methods = ['POST'])
